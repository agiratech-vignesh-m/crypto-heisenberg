  ...
  return await loanContract.methods.dashboardView(_studentID)
    ...
    .then((studentDetails1) => {
      let sDetails = {
          ...
          remainingInstallmentMonths: studentDetails1.remainingInstallmentMonths
      }
...
export const repayLoanSetup = async (conn_provider, w_address, _studentID, _amount) => {
  ...
  if (provider) {
    const web3 = new Web3(provider);
    let quorumToWei = web3.utils.toWei(_amount.toString(), 'ether');
      return await loanContract.methods.repayLoan(
        _studentID, quorumToWei).send({
      from: w_address
    })
  }
}
